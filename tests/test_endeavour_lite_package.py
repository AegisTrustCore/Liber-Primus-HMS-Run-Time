...
class EndeavourLitePackageTests(unittest.TestCase):
    def test_portable_envelope_is_deterministic_and_checksummed(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            (stage / "HMS-Endeavour-Runtime.exe").write_bytes(b"gui"); (stage / "HMS-Endeavour-Runtime-CLI.exe").write_bytes(b"cli")
            ...
            with zipfile.ZipFile(first) as archive:
                self.assertIsNone(archive.testzip()); self.assertIn("canonical/LP-75-IMAGES-v1.0.0.json",archive.namelist()); self.assertIn("HMS-Endeavour-Runtime-CLI.exe",archive.read("SHA256SUMS").decode())
