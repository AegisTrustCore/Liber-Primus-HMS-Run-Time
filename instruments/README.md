...
| HMS Expedition Verifier | IN DEVELOPMENT — 0.3.0 | Sealed-service/client deployment candidate; campaign closed; no customer download | Observer |
| Corpus Manifest Verifier | IN DEVELOPMENT — 0.2.0-dev | Nautical UI and embedded 75-page viewer under new qualification; no customer download | Observer |
| Public GP29 Calculator | RELEASED — 0.1.1 | Public Windows desktop/CLI download with approved manifest and checksums | Observer |
| HMS Endeavour Lite | IN DEVELOPMENT — 0.2.0-dev | Bounded public workstation being separated from member Runtime capability | Observer |
| HMS Endeavour Runtime Environment | INTERNAL TESTING — 1.1.0-dev | Private member visual research deck; no public GitHub download or current Patreon package | Navigator and above by capability |
| Advanced GP Laboratory | PLANNED | No service | Cartographer |
| Hosted Liber Runtime Beta | PLANNED | No hosted service; the local Runtime Environment is the current workstation | Navigator |
...
The validator and source-audit inventory were released in `v0.1.0` specifically as developer tools. They require CPython and are not customer-ready applications. GP29 v0.1.1 is the first released customer-ready Windows instrument. Endeavour Lite is the bounded future public workstation; the complete Runtime Environment is a separately packaged member product and a future hosted Liber Runtime remains distinct. Expedition 001 remains closed until its deployed service and configured client pass the joint release gate.
